...
do
  ...
  $QSUB -t 1-400 -N filter_${d/.pickle/}_connected_cliques$JOBSUFFIX bash wrap_compile.sh filter_potentially_connected_cliques.py $PROBLEM.problem $BASE/$PROBLEM $MAXLEN ${d/.pickle/}
  $QSUB -N deduplicate_connected -hold_jid filter_${d/.pickle/}_connected_cliques$JOBSUFFIX bash wrap_compile.sh deduplicate_cliques.py $BASE/$PROBLEM potentially_connected_cliques/${d/.pickle/} collated_potentially_connected_cliques/${d/.pickle/}
done
